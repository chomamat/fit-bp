...
class VggLoss(nn.Module):
    def __init__(self, output_layer=-18, vgg_factor=0.00001):
    ...
    def upChannel(self,x):
        """ VGG19 works with 3 channels, but radar images are only grayscale.
            It empirically works, just concatenate the image in the 
            channels dimension.
        """
        out = x
...
class CombinedLoss(nn.Module):
    def __init__(self, vgg_layer=-18, vgg_factor=0.00001):
