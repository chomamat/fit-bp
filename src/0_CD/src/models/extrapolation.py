...
class Model(nn.Module):
    ...
    def predict(self,x,num_steps):
        """ Mehod predict() takes care of extrapolation num_steps steps into the future.
            The idea is that output of the network will be again used as the input.
        """
        assert x.shape[1] == 3
        ...
        for i in range(def_shape[0]):                   # process each sequence in batch separately
            tmp = []
            x_tmp = x[i:i+1]

            for i in range(num_steps):                  # loop over each prediction step
