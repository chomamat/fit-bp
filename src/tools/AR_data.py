...
class TimeSeriesData:
    ...
    def reshape_data(self):
        self.x = np.reshape([self.y[i:i + self.num_prev] for i in range(
            self.num_datapoints-self.num_aft)], (-1, self.num_prev))
        self.y = np.reshape([self.y[i:i + self.num_aft] for i in
            range(self.num_prev,self.num_prev+self.num_datapoints-self.num_aft)],
            (-1, self.num_aft))
        # np.copy(self.y[self.num_prev:])
